...
>
> | 功能类型 | 使用场景 | 对应模式 | 获取与配置方法（⚠️ 注意客户端差异） |
> | :--- | :--- | :--- | :--- |
> | **短信通知推送（单向）** | 手机收到短信/未接来电/电量告警，自动推送到群聊 | **群自定义机器人 (Webhook)** | ⚠️ **建议在电脑端桌面客户端操作**：飞书与钉钉移动端群设置默认隐藏了「自定义机器人」入口！打开电脑端群聊 → 右上角群设置 →【机器人】→【添加机器人】→ 选择【自定义机器人】，复制生成的 Webhook 地址与密钥填入 App【通用转发 / 多通道管理】中。 |
> | **远程控制发信（双向免公网）** | 在群或私聊中发送 `/发信 10086 查询` 驱动备用机 SIM 卡发短信 | **企业微信智能机器人 / 飞书 & 钉钉 Stream 官方长连接** | 1. **企业微信端**（[官方文档 101463](https://developer.work.weixin.qq.com/document/path/101463)）：<br>• 电脑后台：登录企微后台 (work.weixin.qq.com)，进入「安全与管理 → 管理工具 → 智能机器人」，点击「创建智能机器人」，选择【API模式】并勾选【使用长连接】获取 `Bot ID` 与 `Secret`；<br>• 手机 App：打开企业微信 App →「通讯录」→「智能机器人」→「创建智能机器人」同样可直连创建；<br>• 在 App 中填入凭证保存显示「已连接」，拉入群或单聊即可 `@机器人 /发信 ...`；<br>2. **飞书端**：在开放平台 (open.feishu.cn) 创建应用，在「开发配置 → 事件与回调」进入**【事件配置】**Tab（⚠️ 切勿配成回调配置），选择「使用长连接接收事件」并订阅 `im.message.receive_v1` 事件；手机端连通后再回后台点验证；<br>3. **钉钉端**：在开放平台 (open.dingtalk.com) 开启机器人并选择「Stream 模式」。 |
