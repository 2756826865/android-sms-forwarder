...
> ### 💡 钉钉与飞书机器人配置指南（单向推送 vs 远程发信）
>
> | 功能类型 | 使用场景 | 对应模式 | 获取与配置方法（⚠️ 注意客户端差异） |
> | :--- | :--- | :--- | :--- |
> | **短信通知推送（单向）** | 手机收到短信/未接来电/电量告警，自动推送到群聊 | **群自定义机器人 (Webhook)** | ⚠️ **必须在电脑端（PC / Mac 桌面客户端）操作**：飞书与钉钉移动端群设置默认隐藏了「自定义机器人」入口！打开电脑端群聊 → 右上角群设置 →【机器人】→【添加机器人】→ 选择【自定义机器人】，复制生成的 Webhook 地址与密钥填入 App【通用转发 / 多通道管理】中。 |
> | **远程控制发信（双向）** | 在群或私聊中发送 `/发信 10086 查询` 驱动备用机 SIM 卡发短信 | **开放平台自建应用 (Stream 长连接)** | 需在开放平台 (open.feishu.cn / open.dingtalk.com) 创建自建应用并开启机器人能力：<br>1. **飞书端**：在「开发配置 → 事件与回调」进入**【事件配置】**Tab（⚠️ 切勿配成回调配置），选择「使用长连接接收事件」并添加「接收消息 v2.0」(`im.message.receive_v1`) 事件；手机端填入凭证保存显示「已连接」后，回到后台点击验证；<br>2. **群聊使用**：在电脑端将该自建应用拉入群聊，即可在群内 `@机器人 /发信 ...` 进行发信。 |
