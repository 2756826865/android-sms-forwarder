...
> | **远程控制发信（双向免公网）** | 在群或私聊中发送 `/发信 10086 查询` 驱动备用机 SIM 卡发短信 | **企业微信智能机器人 / 飞书 & 钉钉 Stream 官方长连接** | 1. **企业微信端**：企业管理员登录企微管理后台 (work.weixin.qq.com)，进入「应用管理 → 机器人」创建智能机器人，获取 `Bot ID` 与 `Secret`；在 App 中填入保存显示「已连接」，拉入群或单聊即可 `@机器人 /发信 ...`；<br>2. **飞书端**：在开放平台 (open.feishu.cn) 创建应用，在「开发配置 → 事件与回调」进入**【事件配置】**Tab（⚠️ 切勿配成回调配置），选择「使用长连接接收事件」并订阅 `im.message.receive_v1` 事件；手机端连通后再回后台点验证；<br>3. **钉钉端**：在开放平台 (open.dingtalk.com) 开启机器人并选择「Stream 模式」。 |
