...
/** Serializes incoming SMS work in a foreground service so it survives OEM screen-off freezing. */
class IncomingSmsService : Service() {
    ...
    private fun processIncoming(intent: Intent) {
        ...
        val forwardingConfig = PushPlusConfig(applicationContext)
        forwardingConfig.lastReceiverStatus = "已收到${intent.action?.substringAfterLast('.').orEmpty()}，正在写入短信库"
        ...
        // Only a verified provider insert is allowed to suppress the duplicate SMS_RECEIVED action.
        markPersisted(fingerprint)
        SmsRecoveryWorker.markObserved(applicationContext, receivedAt)
        Log.i(TAG, "SMS persisted: id=$insertedMessageId thread=$resolvedThreadId action=${intent.action}")
        ...
        if (forwardingConfig.enabled) {
            PushPlusWorker.enqueue(this, address, body, receivedAt, subscriptionId, uniqueId)
        }
        if (MultiForwardConfig(this).anyEnabled()) {
            MultiChannelForwardWorker.enqueue(this, address, body, receivedAt, subscriptionId, uniqueId)
        }
        forwardingConfig.lastReceiverStatus = "已接收并写入短信库，短信ID：$insertedMessageId，发送方：$address"
    }
    ...
    private fun isFiltered(address: String, body: String): Boolean {
        ...
        if (!isWhitelisted && baseConfig.blockUnknownNumbers) {
            ...
            if (SimpleContactsHelper(this).existsSync(address, cursor) == ContactLookupResult.NotFound) return true
        }
        return false
    }
    ...
    private fun fingerprint(address: String, body: String, sentAt: Long, subscriptionId: Int): String {
