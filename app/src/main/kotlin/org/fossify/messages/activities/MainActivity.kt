...
class MainActivity : SimpleActivity() {
    ...
    private var messageCenterViewModel: MessageCenterViewModel? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.homeSwitchDeveloperUi.setOnClickListener {
    ...
    private fun setupGatewayDeveloperUi() {
        ...
        val dvm = DashboardViewModel(GetDashboardStatsUseCase(dashboardRepo)).also { dashboardViewModel = it }
        val mvm = MessageCenterViewModel(GetMessageHistoryUseCase(messageRepo)).also { messageCenterViewModel = it }
        val diagVm = DiagnosticsViewModel(
        ...
        ).also { diagnosticsViewModel = it }
        val cvm = ConversationsViewModel(application).also { conversationsViewModel = it }

        setContent {
            GatewayTheme {
                GatewayApp(
                    dashboardViewModel = dvm,
                    messageCenterViewModel = mvm,
                    diagnosticsViewModel = diagVm,
                    conversationsViewModel = cvm,
                    onRequestDefaultSmsRole = { requestDefaultSmsApp() },
                    onSwitchToClassic = {
                        config.useGatewayDeveloperUi = false
                        recreate()
                    }
                )
            }
    ...
    private fun refreshHomeUnreadSubtitleFromAdapter() {
        ...
        updateHomeUnreadSubtitle(adapter.currentList.filterIsInstance<Conversation>())
    }
    ...
    private fun getCreateNewContactShortcut(appIconColor: Int): ShortcutInfo {
        ...
        return ShortcutInfo.Builder(this, "new_conversation")
            ...
            .setIcon(if (bmp != null) Icon.createWithBitmap(bmp) else null)
            .setIntent(intent)
            .setRank(0)
            .build()
    }
